...
class ResultControleur{
    ...
    public function insertReponseContoleur($idquestion,$idreponse) {

        $resultModel = new Resultat();
        $resultModel->idquestion = $idquestion;
        $resultModel->idreponse = $idreponse;

        $this->insertReponse = $resultModel->insertReponseUser();
    }

    public function scoreContoleur() {
        $scoreModel = new Resultat(); 

        return $scoreModel->score();
    }

    public function deleteControleur() {
        $deleteModel = new Resultat(); 
    ...
    public function Correction(){
        ...
        foreach ($tousLesStatuts as $statut) {
            ...
            $justifications = $objetResult->justification($statut['idquestion']);
            echo '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="' . ($statut['reponsestatut'] == 0 ? 'red' : 'green') . '" class="bi ' . ($statut['reponsestatut'] == 0 ? 'bi-x-circle' : 'bi-check-circle') . '" viewBox="0 0 16 16">';
            echo '<path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14"/>';
            if ($statut['reponsestatut'] == 0) {
            ...
            echo '</svg>' . $statut['idquestion']."-".$statut['reponsecontent']."<br>";
    
            if ($statut['reponsestatut'] == 0 && !empty($justifications)) {
                echo "Correction: ";
